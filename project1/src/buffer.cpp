...
std::shared_ptr<BlockData> BlockBuffer::from_buffer(const std::streamoff &offset) {
...
void BlockBuffer::to_buffer(const std::streamoff &offset, const std::shared_ptr<BlockData> &block) {
...
void BlockBuffer::update_buffer(const std::streamoff &offset, const BlockData &block) {
  auto it = _blocks.find(offset);
  if (it != _blocks.end()) {
